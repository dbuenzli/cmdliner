...
let generate_dune_config ~shells ~toolname ~standalone_completion:standalone =
  with_binary_stdout @@ fun () ->
  let shell_completion (module Shell : SHELL) =
    let completion_file = toolname ^ "_completion." ^ Shell.name in
    Printf.printf
{|(rule
  (with-stdout-to
    %s
  (run cmdliner tool-completion %s %s%s)))
...
|}
    ...
    Shell.name
    toolname
    (if standalone then " --standalone-completion" else "")
    completion_file
    (Filename.concat Shell.sharedir (Shell.tool_script_name ~toolname:toolname))
  in
  let manpage cmd =
    let man_basename =
      let exec = if cmd = "" then toolname else String.concat " " [toolname; cmd] in
      (String.map (function ' ' -> '-' | c -> c) exec)
    in
    Printf.printf
{|(rule
...
|}
    ...
    man_basename 1
  in
  match get_tool_commands toolname with
  | Ok cmds -> print_endline "; generated by cmdliner generate-dune-config";
               List.iter manpage ("" :: cmds);
               List.iter shell_completion shells;
               Cmdliner.Cmd.Exit.ok
...
let dune_config_cmd =
  let doc = "Output dune configuration for installing tool support files" in
  let man =
    [ `S Manpage.s_description;
      `P "$(cmd) outputs dune install stanzas and rules to generate \
          cmdliner tool completion scripts and manpages. \
          These can be used with a dynamic_include stanza in a dune file to allow \
          $(i,dune build @install) to install the support files."
    ]
  in
  Cmd.make (Cmd.info "generate-dune-config" ~doc ~man) @@
  let+ shells = shells_opt
  and+ toolname = toolname_pos0
  and+ standalone_completion in
  generate_dune_config ~shells ~toolname ~standalone_completion
...
let cmd =
  ...
  [generic_completion_cmd; tool_commands_cmd; dune_config_cmd; tool_completion_cmd; install_cmd]
